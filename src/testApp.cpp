...
void testApp::setupControls()
{
	...
	panel.setup("3d model", ofGetWidth()-340, 10, 320, 730);
	...
	panel.addSlider("smooth add points","smooth_add_points",4,0,20,true);
	panel.addSlider("smooth average before","smooth_average_before",1,0,100,true);
	panel.addSlider("smooth average after","smooth_average",3,0,100,true);
...
void testApp::loadTag(string name)
{
	...
	int avgNumB = panel.getValueI("smooth_average_before");
	for( int i = 0; i < avgNumB; i++)
		tag.average();					// add or remove averging for more smoothings
	
	int smoothNum = panel.getValueI("smooth_add_points");
	if( smoothNum > 0 )
		gSmoother.smoothTag(smoothNum, &tag);	// change 3 to 4 for more but dont exceed this. 2 is too small
	
	int avgNum = panel.getValueI("smooth_average");
